...
export function selectTodosFromTodoList() {
  const selectByStatus = useCallback((todoList: Todo[], status: TodoFilterStatus) => {
  ...
  const selectByQuery = useCallback((todoList: Todo[], query: string | undefined) => {
  ...
  return { selectByStatus, selectByQuery };
}
